...
function setWifi()
    ...
    if code == 0 then
        if index == 1 or index == 2 then
            local succeed = XQWifiUtil.setWifiBasicInfo(index, ssid, password, encryption, channel, txpwr, hidden, on, bandwidth, nil, txbf, weakenable, weakthreshold, kickthreshold)
...
function setAllWifi()
    ...
    if on1 == 0 then
        ...
        if ssid1 and on1 then
            ...
            if code1 == 0 then
                succeed = XQWifiUtil.setWifiBasicInfo(1, ssid1, password1, encryption1, channel1, txpwr1, hidden1, on1, bandwidth1, bsd, "0", weakenable, weakthreshold, kickthreshold)
            else
                code = code1
            end
        end
    ...
    if on2 == 0 then
        ...
        if ssid2 and on2 then
            ...
            if code2 == 0 then
                succeed = XQWifiUtil.setWifiBasicInfo(2, ssid2, password2, encryption2, channel2, txpwr2, hidden2, on2, bandwidth2, bsd, txbf, weakenable, weakthreshold, kickthreshold)
            else
                code = code2
            end
        end
...
function setWan6()
    ...
    if XQFunction.isStrNil(wanType)
        ...
        elseif wanType == "static" then
            ...
            if string.find(ip6addr, "/") == nil then
                ip6addr = ip6addr .. '/64'
            end
            local static
            local ip6prefix = ip6prefix .. '1/' .. ip6assign
            local ipv6_check = string.format("/usr/sbin/ipv6_subnet_check.sh %s %s", ip6addr, ip6prefix)
            local t = io.popen(ipv6_check)
            local check_result = t:read("*a")
            t:close()
